...
export default function CreateProduct() {
  ...
  const fetchFiltersData = async () => {
    try {
      ...
      if (brandsData.length === 0) {
        console.warn('No brands found. Please create brands first.');
      }
      ...
      alert('Failed to load categories and brands. Please refresh the page.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Build product data - only include required fields and fields with values
      const productData = {
        name: formData.name,
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">Add a new product to your catalog</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              {renderFormSection('Product Images', (
              ...
              {renderFormSection('Organization', (
                <div className="space-y-4">
                  <div>
                    ...
                    {categories.length === 0 && (
                      <p className="mt-1 text-xs text-yellow-600">
                        ⚠️ No categories available. <Link href="/categories/create" className="underline">Create one first</Link>
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {brands.length === 0 && (
                      <p className="mt-1 text-xs text-yellow-600">
                        ⚠️ No brands available. <Link href="/brands/create" className="underline">Create one first</Link>
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="flex justify-end space-x-4">
            ...
            <button type="submit" disabled={loading || categories.length === 0 || brands.length === 0}
              ...
              title={categories.length === 0 || brands.length === 0 ? 'Please create categories and brands first' : ''}>
              {loading && <LoadingSpinner size="sm" />}
              {categories.length === 0 || brands.length === 0 ? 'Create Categories & Brands First' : 'Create Product'}
            </button>
          </div>
        </form>
      </div>
