...
import { formatCurrency, formatDate, handleAPIError, getStatusColor } from '@/lib/utils';
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {coupons.map((coupon) => (
                    <tr key={coupon.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{getCouponTypeDisplay(coupon.type)}</div>
                        <div className="text-sm font-semibold text-green-600">{getCouponValue(coupon)}</div>
                        {coupon.minPurchase > 0 && (
                          <div className="text-xs text-gray-500">Min: {formatCurrency(coupon.minPurchase)}</div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          getStatusColor(coupon.isActive && !isExpired(coupon.expiresAt) && !isUsageLimitReached(coupon) ? 'active' : 'inactive')
                        }`}>
                          {coupon.isActive && !isExpired(coupon.expiresAt) && !isUsageLimitReached(coupon) ? 'Active' : 'Inactive'}
                        </span>
                      </td>
