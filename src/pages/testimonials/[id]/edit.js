...
export default function EditTestimonial() {
  ...
  const fetchTestimonial = async () => {
    try {
      ...
      setFormData({
        ...
        testimonial_date: testimonial.testimonial_date ? new Date(testimonial.testimonial_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        is_verified_purchase: testimonial.is_verified_purchase || false,
      ...
      if (testimonial.customer_photo) {
        setPhotoPreview(testimonial.customer_photo);
      }
    } catch (error) {
  ...
  return (
    <>
      <Head>
        <title>Edit Testimonial - ENOT Admin</title>
      </Head>
